...
internal sealed class MongoBookRepository(IMongoDatabase database): IBookRepository
{
    ...
    {
        RegistrationWindowDocument document = await RegistrationWindows.Find(w => w.Id == RegistrationWindowId)
    ...
    {
        return await Books.Find(b => b.Isbn == isbn)
    ...
    {
        BookDocument? document = await Books.Find(b => b.Id == id)
                                            .SingleOrDefaultAsync(cancellationToken);

        return document is null
            ? null
            : Book.FromPersistence
                (document.Id, document.Isbn, document.Title, document.Author, document.CopiesAvailable);
    }
